...
@app.get("/api/_debug/oura-fields")
async def debug_oura_fields(request: Request):
    """One-off diagnostic: dump field keys from the user's recent Oura sleep,
    daily_sleep, daily_resilience, and daily_stress records, with any
    breathing/respiration/disturbance/variation fields highlighted with their
    values. Used to figure out the real name (or absence) of Oura's BDI in
    the public API. Auth-gated to the user themselves — only returns the
    requesting user's own Oura data.

    REMOVE this endpoint after the BDI question is resolved.
    """
    session = _require_session(request)
    user_id = session["user_id"]
    ...
    if not session.get("access_token"):
        ...
        if db:
            try:
                ...
                pass
    if not session.get("access_token"):
        raise HTTPException(status_code=400, detail="No Oura connection")
    ...
    start = (_d.today() - _td(days=3)).isoformat()

    OURA_BASE = "https://api.ouraring.com/v2/usercollection"
    endpoints = {
        "sleep":            f"{OURA_BASE}/sleep?start_date={start}&end_date={end}",
        "daily_sleep":      f"{OURA_BASE}/daily_sleep?start_date={start}&end_date={end}",
        "daily_resilience": f"{OURA_BASE}/daily_resilience?start_date={start}&end_date={end}",
        "daily_stress":     f"{OURA_BASE}/daily_stress?start_date={start}&end_date={end}",
    }
        ...
        for name, url in endpoints.items():
            try:
                ...
                if not isinstance(last, dict):
                    out[name] = {"shape": type(last).__name__}
                    continue
                keys = sorted(last.keys())
                # Find any field whose name hints at breathing / variability,
                # plus any nested contributors keys (Oura wraps derived
                # signals in `contributors`).
                terms = ("breath", "respir", "disturb", "vari", "apnea", "snor")
                suspicious: dict = {}
                for k, v in last.items():
                    if any(t in k.lower() for t in terms):
                        suspicious[k] = v
                contribs = last.get("contributors")
                if isinstance(contribs, dict):
                    suspicious["_contributors_keys"] = sorted(contribs.keys())
                    for ck, cv in contribs.items():
                        if any(t in ck.lower() for t in terms):
                            suspicious[f"contributors.{ck}"] = cv
                out[name] = {
