...
def get_todays_move(request: Request, date: Optional[str] = None):
    """One concrete, actionable recommendation for the user right now.

    Generated by Coach Al on each call (no caching for v1 — we want it to be
    responsive to actions the user just took). Returns the structured shape
    defined in backend/todays_move.py. Falls back to a default if Haiku is
    unreachable or the JSON is malformed — never 500s on this path because
    the dashboard relies on it.

    Accepts `?date=YYYY-MM-DD` (the client's local date) — Render runs UTC,
    so without this the endpoint thinks it's tomorrow after ~4pm PT.
    """
    session  = _require_session(request)
    user_id  = session["user_id"]
    profile  = _get_profile(user_id) or {}
    ...
    try:
        ...
        if db:
            ...
            res = (db.table("daily_checkins")
                     .select("mood")
                     .eq("user_id", user_id)
                     .eq("date", yesterday)
                     .limit(1).execute())
            if res.data:
                yesterday_mood = (res.data[0] or {}).get("mood")
    except Exception:
        pass

    # Minimal health context — goal pace + today's nutrition + body. This is
    # purposefully narrower than the briefing's context. Today's Move needs
    # to be FAST and decisive, not a synthesis.
    try:
        active_goal = gl.get_active_with_pace(user_id) or {}
    except Exception:
        active_goal = {}
    try:
        nutr_body = nutr.coach_snapshot(user_id, today_iso) or {}
    except Exception:
        nutr_body = {}

    # Today's headline numbers — steps + sleep score etc. — best-effort.
    today_data: dict = {}
    try:
        ah_today = ah.get_day(user_id, today_iso) or {}
        for k in ("steps", "sleep_hours", "hrv", "resting_hr", "active_calories"):
            if ah_today.get(k) is not None:
                today_data[k] = ah_today[k]
    except Exception:
        pass
    ...
    health_context = {
        ...
        "nutrition":   nutr_body.get("nutrition"),
        "body":        nutr_body.get("body"),
    }

    # Generate (or fall back) — never 500.
    try:
        import todays_move as tm
        move = tm.generate_todays_move(health_context, profile, yesterday_mood)
    except Exception as e:
        print(f"[todays-move] generation failed: {e}", flush=True)
        import todays_move as tm
        move = tm.default_move()
