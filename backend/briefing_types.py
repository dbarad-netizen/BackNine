...
def overlay_for(briefing_type: str, ctx: Optional[dict] = None) -> str:
    ...
    if briefing_type == TYPE_LAB_FOCUS:
        return (
            "\n=== TODAY'S FOCUS: RECENT LAB UPLOAD ===\n"
            "The user uploaded a lab result recently. If the data "
            "sections above INCLUDE a lab/lab_results block:\n"
            "  • Paragraph 1: name ONE marker worth discussing at their "
            "    next visit. Choose based on what's out of the reference "
            "    range OR what's changed meaningfully vs the prior draw. "
            "    Use the number and unit, and give one sentence of "
            "    context ('flagged high vs the reference range' — don't "
            "    diagnose, don't prescribe, don't interpret causally).\n"
            ...
            "  • If multiple flagged markers, pick the highest-signal "
            "    one. Don't list them all — that's what the Doctor "
            "    Report is for.\n"
            "\n"
            "IF the data sections DO NOT include a lab/lab_results "
            "block (this happens when the upload triggered the focus "
            "but the labs haven't propagated to your context yet): "
            "pivot SILENTLY to a normal last-night synthesis using "
            "sleep, HRV, readiness, and activity. Do NOT mention that "
            "you were expecting labs. Do NOT ask the user which they'd "
            "prefer. Just deliver a useful briefing from what you have. "
            "David 2026-08-07 saw a briefing that said 'I don't see a "
            "lab result payload in today's context, though the briefing "
            "type is marked as LAB FOCUS' — that leaked internal machinery "
            "and asked the user to make a decision they shouldn't have "
            "to make. Never do that again.\n"
        )
